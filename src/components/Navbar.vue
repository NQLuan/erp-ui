            ...
            <ul id="mydrop" class="nav justify-content-end navbar-nav ml-auto">
                <li class="nav-item" @click="toggle">
                    <button class="btn dropdown-toggle pr-4">
                        <img v-if="imageurl !== 'null' & imageurl !== 'undefined'" class="img-fluid" :src="imageurl" />
                        <img v-if="imageurl === 'null' || imageurl === 'undefined'" class="img-fluid" src="../static/images/icon.png"/>
                    </button>
                </li>
                <ul ref="dropdown" class="collapse list-unstyled ml-auto rounded my-drop-content">
                    <div>
                        <li class="ml-auto mr-2 pl-2 mt-2 text-muted">
                            <router-link class="my-2 button" :to="'/profile/'+profileId" >
                                {{ email }}
                            </router-link>
                        </li>
                        <hr />
                        <li class="ml-auto pl-2 mr-2">
                            <router-link class="my-2 button" to="/changepassword"  @click="toggle()">
                                Change password
                            </router-link>
                        </li>
                        <li class="ml-auto pl-2 mr-2">
                            <router-link class="my-2 button" :to="'/profile/'+profileId" @click="toggle()">
                                My profile
                            </router-link>
                        </li>
                         <li class="ml-auto pl-2 mr-2">
                            <router-link class="my-2 button" :to="'/photos/'+profileId">
                                My Photos
                            </router-link>
                        </li>
                        <hr />
